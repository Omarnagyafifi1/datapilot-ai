...
import { History, Search, Filter, Download, Terminal, Clock, Loader2 } from 'lucide-react';
    ...
    <div className="p-12 max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="mb-12 flex items-end justify-between">
        ...
        <div className="flex gap-4">
          <button className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-xs font-mono hover:bg-white/10 transition-all">
      ...
      <div className="glass rounded-2xl border-white/5 overflow-hidden">
        <div className="grid grid-cols-12 p-4 border-b border-white/5 bg-white/5 text-[10px] font-mono font-bold text-muted uppercase tracking-widest">
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button className="w-8 h-8 rounded border border-white/10 flex items-center justify-center text-xs text-muted hover:border-cyber-cyan hover:text-white transition-all">1</button>
        </div>
      </div>
    </div>
  );
...
function ArchiveItem({ intent, status, latency, time }) {
  return (
    <div className="grid grid-cols-12 p-5 items-center hover:bg-white/[0.02] transition-colors group cursor-pointer">
      <div className="col-span-5 flex items-center gap-4">
        <div className={`w-1.5 h-1.5 rounded-full ${status === 'SUCCESS' ? 'bg-cyber-cyan shadow-[0_0_8px_rgba(0,242,255,0.5)]' : 'bg-red-500'}`} />
        <span className="text-sm text-white/80 group-hover:text-white transition-colors truncate pr-4">{intent}</span>
      </div>
